package game.listeners.tutorial
{
	...
	public class TutorialScreenListener3 extends ScreenListener
	{
		public function TutorialScreenListener3(screen:Screen, videoType:String = Video.TYPE_SONG)
		{
			super(screen, videoType);
			Main.appendMessage("Tutorial Screen Listener 3");
		}
		...
		{
			var tutorialScreen4:Screen;
			if (this.videoType == Video.TYPE_SONG)
				tutorialScreen4= new Screen(Images.getInstance().image4B, Sounds.getInstance().sound4B);
			else
				tutorialScreen4= new Screen(Images.getInstance().image4, Sounds.getInstance().sound4);
			tutorialScreen4.addEventListener(Event.ADDED, new TutorialScreenListener4(tutorialScreen4, this.videoType).loaded);
			Main.instance.replaceScreen(tutorialScreen4);
		}
	}
}