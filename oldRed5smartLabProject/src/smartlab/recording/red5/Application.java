...
public class Application extends ApplicationAdapter
{
	private static final Logger					log						= Red5LoggerFactory.getLogger(
																				Application.class, "smartLabProject");
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.red5.server.adapter.ApplicationAdapter#connect(org.red5.server.api .IConnection,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.red5.server.adapter.ApplicationAdapter#disconnect(org.red5.server .api.IConnection,
	...
	private static String getNewStream()
	{
		...
		for (int i = 0; i < 20; i++)
		{
			generated += ALLOWED_CHARACTERS.charAt((int) Math.round(Math.random() * (ALLOWED_CHARACTERS.length() - 1)));
		}
		return generated;
	}

	/**
	 * Frees a stream that is no longer used. Happens when the recording is finished
	 * 
	 * @param stream
	...
	public String record(String streamName, int type)
	{
		...
		try
		{
			ClientBroadcastStream stream = (ClientBroadcastStream) this.getBroadcastStream(scope, streamName);
			streamFileNames.put(streamName, file + ".flv");
			stream.saveAs(file, false);
			log.debug("ALL IS GOOD, Recording started");
		...
		catch (Exception e)
		{
			...
			log.debug("ERROR IN Stream.SaveAs" + e.getMessage(), e);
			return null;
		}
		return file;
	}
	...
	{
		return Integer.parseInt(time.substring(0, time.indexOf(":"))) * 60 * 60 * 1000
				+ Integer.parseInt(time.substring(time.indexOf(":") + 1, time.lastIndexOf(":"))) * 60 * 1000
				+ Integer.parseInt(time.substring(time.lastIndexOf(":") + 1, time.indexOf("."))) * 1000
	...
	public String getDay(String participantId)
	{
		int number = 1;
		File f = new File(FilenameGenerator.recordPath + participantId + File.separator);
		if (!f.exists())
		{
			f.mkdirs();
		}
	...
	public void saveDayData(String participantId, String day, String data, String emailAddress)
	{
			...
			String video, String video_trial)
	{
		...
		if (type.equals(OUTPUT_TYPE_H264))
		{
			...
			outputFFMPEG = new String[] { "-vcodec", "copy", "-acodec", "libfaac", "-ar", "22050", "-ab", "64000",
					"-ac", "1", newFile.getAbsolutePath() };
		}
		else
			return null;
	...
	//		audioDelay *= -1;
		...
		try
		{
			...
			Scanner stream = new Scanner(process.getErrorStream());
			while (stream.hasNextLine())
				log.debug("FFMPEG:" + stream.nextLine());
			stream.close();
			process.waitFor();
	...
	/*
	 * private String getStreamForFileName(String fileName) { String streamName = fileName.substring(0,
	 * fileName.lastIndexOf("_")); return streamName; }
	 */
	...
	public void saveFile(String streamName)
	{
		...
		File newFile = new File(FilenameGenerator.recordPath + fileName.substring(0, fileName.lastIndexOf("."))
				+ ".mp4");
		ProcessBuilder p = new ProcessBuilder(FFMPEG, "-i", oldFile.getAbsolutePath(), "-vcodec", "copy", "-an",
				newFile.getAbsolutePath());
		try
		{
			Process process = p.start();
	...
	/*
	 * @SuppressWarnings("unchecked") private static <T> T[] concatenateArrays(T[] a, T[] b) { T[] c = (T[])
	 * Array.newInstance(a.getClass(), a.length + b.length);
	 * 
